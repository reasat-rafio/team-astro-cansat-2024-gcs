...
>
  ...
  {#if $snapshot.matches('maximize')}
    <div
      transition:slide
      class="p-2 bg-black/40 backdrop-blur-lg h-[450px] overflow-y-auto"
    >
      <!-- HISTORY -->
      {#if !!$snapshot.context.commandHistory?.length}
        <div class="flex flex-col gap-y-1'">
          {#each $snapshot.context.commandHistory as { text, timestamp }}
            <div class="flex gap-x-2">
              <Prompt />
              <div class="flex-1 flex-wrap break-words w-fit">{text}</div>
            </div>
          {/each}
        </div>
      {/if}
