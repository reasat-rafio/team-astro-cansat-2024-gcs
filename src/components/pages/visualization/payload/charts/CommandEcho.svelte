<script lang="ts">
  ...
  async function updateGraph() {
    if (chart) {
      chart.data.datasets[0].data.push(
        $commandEchoStore.value[$commandEchoStore.value.length - 1]
      );
      chart.data.labels?.push(
        $commandEchoStore.time[$commandEchoStore.time.length - 1]
      );
  ...
  $: $commandEchoStore, updateGraph();
</script>

<section>
  <div class="flex">
    <h4 class="h6 ml-5 flex-1 text-tertiary-500">Command Echo</h4>
  ...
  <div bind:this={containerEl} class="overflow-x-scroll scroll-smooth">
    <div
      class="h-[300px]"
      style="width: {500 + $commandEchoStore.value.length * 50}px; "
    >
      <Line
        bind:chart
        data={{
